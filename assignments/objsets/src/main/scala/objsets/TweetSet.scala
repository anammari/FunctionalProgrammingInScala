...
class Empty extends TweetSet {
  ...
  override def mostRetweeted: Nothing = throw new NoSuchElementException("Nil.tweet")
...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  override def mostRetweeted: Tweet = {
    if(((left union right) union this) == this) elem
    else {
      val t = ((left union right) union this).remove(elem).mostRetweeted
      if (elem.retweets >= t.retweets) elem else t
    }
  }
  
  override def descendingByRetweet: TweetList = {
    if(((left union right) union this) == this) new Cons(elem, Nil)
    else new Cons(((left union right) union this).mostRetweeted, 
         ((left union right) union this).remove(((left union right) union this).mostRetweeted).descendingByRetweet)
  }
